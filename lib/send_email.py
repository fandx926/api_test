...
from api_test.config.config_test import smtp_user, subject, smtp_server, smtp_password, sender
from django.dispatch import receiver

sys.path.append('..')
from config import *

def send_email1(report_file):
    msg = MIMEMultipart()
    msg.attach(MIMEText(open(report_file, encoding='utf-8').read(), 'html', 'utf-8'))

    msg['From'] = '15765583317@163.com'
    msg['To'] = '2994436996@qq.com'
    ...
    try:
        ...
        smtp.quit()'''
import smtplib#用于建立什么smtp连接
from email.mime.text import MIMEText#邮件需要专门的MIME格式
from email.mime.multipart import MIMEMultipart#混合MIME格式，支持上传附件
from email.header import Header#用户使用中文邮件主题
...
def send_email(report_file):
    ...
    # 添加html格式邮件正文（会丢失css格式） 打开html文件读取报告内容
    msg.attach(MIMEText(open(report_file, encoding='utf-8').read(), 'html', 'utf-8'))
    # 2. 组装Email头（发件人，收件人，主题）
    msg['From'] = '15765583317@163.com'
    msg['To'] = '2994436996@qq.com'
    ...
    att1 = MIMEText(open(report_file, 'rb').read(), 'base64', 'utf-8')  # 从配置文件中读取
    att1["Content-Type"] = 'application/octet-stream'
    att1["Content-Disposition"] = 'attachment; filename="{}"'.format(report_file)  # 参数化一下report_file# filename为邮件中附件显示的名字
    msg.attach(att1)
    # 4. 连接smtp服务器并发送邮件
    try:
